    ...
    function widget($args, $instance) {		
		...
		if ( $title ) {
			if ( $instance['link_title'] === true ) {
				...
				echo vibe_sanitizer($before_title . '<a href="http://twitter.com/' . $instance['username'] . '" class="twitter_title_link" '.$link_target.'>'. $title . '</a>' . $after_title);
			} else {
				echo vibe_sanitizer($before_title . $title . $after_title);
			}
		}
                
		echo vibe_sanitizer($this->really_simple_twitter_messages($instance));
	...
	*/
		...
		foreach ($this->options as $val) { 
			...
			if ($val['type']=='separator') {
				...
				echo '<p>'.$label.'<br />';
				echo '<input class="widefat" id="'.$this->get_field_id($val['name']).'" name="'.$this->get_field_name($val['name']).'" type="text" value="'.(isset($instance[$val['name']])?esc_attr(($instance[$val['name']])):'').'" /></p>';
			} else if ($val['type']=='checkbox') {
				$checked = isset($instance[$val['name']]) ? 'checked="checked"' : '';
				echo '<input id="'.$this->get_field_id($val['name']).'" name="'.$this->get_field_name($val['name']).'" type="checkbox" '.$checked.' /> '.$label.'<br />';
			}else if ($val['type']=='select') {
                                echo '<p>'.$label.'<br />';
				$selected = isset($instance[$val['name']]) ? 'checked="checked"' : '';
