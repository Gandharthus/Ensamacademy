...
//			$r['customs'] =[{"name":"course","val":1242},{"name":"buddydrive_course","val":1}]
    ...
    function buddydrive_get_usercourses(){
    	...
    	if(empty($courses)){
    		...
    		<select id="buddydrive_course" class="buddydrive-customs">
    			...
    			<option value="<?php echo vibe_sanitizer($id); ?>"><?php echo vibe_sanitizer($course,'text'); ?></option>
    			<?php } ?>
    		</select>
    		<?php
    	}
	...
	function privacy_status($args){
		if($args[1] == 'course'){
			$args[0] = '<i class="vicon vicon-stamp"></i>'.__('Course Only','vibe');
		}
		return $args;
	}
...
if ( in_array( 'buddydrive/buddydrive.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) )  || (function_exists('vibe_check_plugin_installed') && vibe_check_plugin_installed('buddydrive/buddydrive.php'))) {
