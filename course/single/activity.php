...
<div class="item-list-tabs activity-type-tabs no-ajax" id="subnav" role="navigation">
	<ul>
		...
		if($flag){ ?>
		<li id="activity-course_<?php echo vibe_sanitizer($bp->current_item,'text'); ?>" class="selected"><a href="#" title="all-activity"><?php _e('All Activity','vibe'); ?></a></li>
		<?php } ?>
		<?php
		if(is_user_logged_in() && bp_course_is_member()){ ?>
		<li id="activity-course_personal_<?php echo vibe_sanitizer($bp->current_item,'text'); ?>"><a href="#" title="<?php _e('My Activity','vibe'); ?>"><?php _e('My Activity','vibe'); ?></a></li>
		<?php } ?>
