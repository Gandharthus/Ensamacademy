			...
			<li><strong><?php _e("Membership Fee", "vibe");?>:</strong>
				<?php
					...
					if($current_user->membership_level->cycle_number > 1) {
						printf(_x('%s every %d %s.', 'Recurring payment in cost text generation. E.g., $5 every 2 months.', 'paid-memberships-pro'), $pmpro_currency_symbol . $level->billing_amount, $level->cycle_number, pmpro_translate_billing_period($level->cycle_period, $level->cycle_number));
					} elseif($current_user->membership_level->cycle_number == 1) {
						printf(_x('%s per %s.', 'Recurring payment in cost text generation. E.g. $5 every month.', 'paid-memberships-pro'), $pmpro_currency_symbol . $level->billing_amount, pmpro_translate_billing_period($level->cycle_period));
					} else {
...
<?php if(pmpro_isLevelRecurring($level)) { ?>
	...
	<?php } else { ?>
		...
		<form id="pmpro_form" class="pmpro_form" action="<?php echo pmpro_url("billing", "", "https")?>" method="post">
			...
			<thead>
				<tr>
					<th colspan="2"><span class="pmpro_thead-msg"><?php printf(__('We accept %s', 'paid-memberships-pro'), $pmpro_accepted_credit_cards_string);?></span><?php _e('Credit Card Information', 'paid-memberships-pro');?></th>
				</tr>
			</thead>
			<tbody>                    
				<tr valign="top">		
