...
<?php if(!empty($pmpro_invoice) && !empty($pmpro_invoice->id)) { ?>		
	...
	<ul>
		...
		<li><strong><?php _e('Account', 'paid-memberships-pro' );?>:</strong> <?php echo vibe_sanitizer($current_user->display_name)?> (<?php echo vibe_sanitizer($current_user->user_email)?>)</li>
		<li><strong><?php _e('Membership Level', 'paid-memberships-pro' );?>:</strong> <?php echo vibe_sanitizer($current_user->membership_level->name)?></li>
		...
		<?php if($pmpro_invoice->getDiscountCode()) { ?>
			<li><strong><?php _e('Discount Code', 'paid-memberships-pro' );?>:</strong> <?php echo vibe_sanitizer($pmpro_invoice->discount_code->code)?></li>
		<?php } ?>
	...
	<div class="pmpro_invoice_details">
		...
		<?php if($pmpro_invoice->accountnumber) { ?>
			<div class="pmpro_invoice-payment-method">
				...
				<p><?php echo vibe_sanitizer($pmpro_invoice->cardtype)?> <?php _e('ending in', 'paid-memberships-pro' );?> <?php echo last4($pmpro_invoice->accountnumber)?></p>
				<p><?php _e('Expiration', 'paid-memberships-pro' );?>: <?php echo vibe_sanitizer($pmpro_invoice->expirationmonth)?>/<?php echo vibe_sanitizer($pmpro_invoice->expirationyear)?></p>
...
<?php 
	...
	<ul>
		<li><strong><?php _e('Account', 'paid-memberships-pro' );?>:</strong> <?php echo vibe_sanitizer($current_user->display_name)?> (<?php echo vibe_sanitizer($current_user->user_email)?>)</li>
		<li><strong><?php _e('Membership Level', 'paid-memberships-pro' );?>:</strong> <?php if(!empty($current_user->membership_level)) echo vibe_sanitizer($current_user->membership_level->name); else _e("Pending", 'paid-memberships-pro' );?></li>
	</ul>	
<?php 
	} 
?>  
